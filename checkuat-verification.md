...
## UAT#12: Proof of Work (customer review/rating removed)

> Customer reviews and ratings were removed from the product. The customer proof-of-work
> photos remain: homeowners attach 1–3 photos before payment (`CustomerProofOfWorkModal`),
> stored in `booking_proof_media` with `submitted_by='customer'`.

| No. | Scenario                                                                                     | Status | Comments                                                                                                                                                                                                                                                                 |
| --- | -------------------------------------------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | Attach proof of work before payment                                                          | P      | `components/booking/CustomerProofOfWorkModal.tsx` (1–3 photos, camera + gallery); gated on `tracking/[id].tsx` (`handlePayment` → `hasCustomerProof`) and `payment/[id].tsx:71-79` (auto-open + pay button blocked until proof attached)                                  |
| 2   | Submit proof photos tied to the booking                                                      | P      | `uploadBookingProof` → `booking-proof` bucket; `attachBookingProof(..., { submittedBy: 'customer' })` (`apiCore.ts:1218-1258`) → `attach_booking_proof` RPC `20260814030000_worker_proof_rating_comment.sql:29-135`                                                        |
| 3   | View the customer proof photos in tracking                                                  | P\*    | `tracking/[id].tsx:358-394` shows all `booking_proof_media` rows (worker + customer) via `fetchBookingProofPhotos`; read covered by `booking_proof_media_party_or_admin_read` + storage `booking_proof_party_or_admin_read` (`20260727130000_trust_pricing_and_reoffer.sql:443-494`) |
| 4   | Review/rating of the provider after service                                                  | N/A    | Customer reviews removed (migration `20260814020000_deprecate_reviews_remove_ratings.sql`). Worker feedback remains via `CompleteJobModal` + `submit_worker_feedback` RPC (`20260813020000_fix_worker_feedback_submission.sql`)                                             |
| 5   | Customers can remove their own proof before confirming payment                               | P      | `delete_booking_proof` now accepts the booking customer for `submitted_by='customer'` rows (`20260816000000_customer_proof_delete.sql`)                                                                                                                                   |
