...
| No. | Scenario                                                                               | Status | Comments                                                                                                                                                                                                                                          |
| --- | -------------------------------------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Attach proof of work and rate the completed work before payment                     | P      | `components/booking/UserReviewWorkModal.tsx`; auto-opened on `tracking/[id].tsx:145-158` and `payment/[id].tsx:72-80` when no review exists                                                                                   |
| 2   | Submit a star rating, written review, proof photos, and recommendation              | P      | `UserReviewWorkModal.tsx:157-220` (proof upload to `booking-proof`, `create_review`, `confirmJobCompletion`); `attach_review_media` `20260721012000_client_operations.sql:3`                                                   |
| 3   | Verify the feedback-submitted confirmation appears                                  | P\*    | Modal closes after submission and proceeds to payment; **no explicit confirmation screen** — the payment screen follows immediately (`payment/[id].tsx:377-386`)                                                                |
...
| 5   | Open the worker's Rate & Review modal and verify the feedback is read-only             | P\*    | Worker reviews are read-only (`(worker)/reviews.tsx`, `components/ReviewsTab.tsx`); worker feedback is submitted via `CompleteJobModal` + `submit_worker_feedback` RPC (`20260813020000_fix_worker_feedback_submission.sql`) |
