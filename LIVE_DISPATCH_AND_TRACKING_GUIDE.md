...
- **Rollback Script**: [`supabase/migrations_archive/manual-rollbacks/20260731020000_worker_arrival_proximity_REVERT.sql`](./supabase/migrations_archive/manual-rollbacks/20260731020000_worker_arrival_proximity_REVERT.sql)
...
- **Rollback Script**: [`supabase/migrations_archive/manual-rollbacks/20260731030000_resilient_matching_configuration_REVERT.sql`](./supabase/migrations_archive/manual-rollbacks/20260731030000_resilient_matching_configuration_REVERT.sql)
...
1. **Check Active Bookings**: Ensure worker account has **0 Active Bookings** (`Active: 0`).
   > _Note_: If the dashboard shows `1 Active` (e.g. an existing booking in progress), complete or cancel that booking first. Workers with active jobs are not assigned new matching offers.
...
1. **Create Service Request**: Log into customer app and create a service request (e.g. _Plumbing_, _Electrical_, or _Cleaning_).
...
4. **50m Proximity Arrival**:
   ...
   - PostGIS validates distance:
     ...
     - If > 50m: App alerts worker: _"You are X meters away. Please get within 50 meters of the destination."_
