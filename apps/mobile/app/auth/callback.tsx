...
export default function AuthCallbackScreen() {
  ...
  const isRecoveryCallback =
    isPasswordRecoveryFlow(parameter(params.flow)) ||
    isPasswordRecoveryFlow(parameter(params.type));

  useEffect(() => {
    let cancelled = false;
    const finish = async () => {
      try {
        if (isRecoveryCallback || isPasswordRecovery) {
          startPasswordRecovery();
          router.replace({
            pathname: PASSWORD_RECOVERY_ROUTE,
            params: {
              flow: 'recovery',
              ...(callbackCode ? { code: callbackCode } : {}),
            },
          } as unknown as Href);
          return;
        }
        ...
        try {
          ...
          router.replace(
            account.role === 'WORKER' ? '/(worker)/profile' : '/(tabs)/profile',
          );
        }
      } catch (error) {
        if (!cancelled)
