...
export default function CategoryScreen() {
  ...
  const categoryName = typeof id === 'string' ? decodeURIComponent(id).toLowerCase() : '';
  ...
  useEffect(() => {
    ...
    void fetchServiceCategories().then((result) =>
      setCategories((result.data ?? []).map((category: any) => ({ id: category.id, label: category.label }))),
    );
  }, []);

  const workers = useMemo(() => providers.filter((worker) => worker.category.toLowerCase().includes(categoryName)), [categoryName, providers]);

  const matchedCategory = categories.find((category) => category.label.toLowerCase() === categoryName);
  const title = categoryName ? categoryName.charAt(0).toUpperCase() + categoryName.slice(1) : 'Workers';

  const openRequestForCategory = () => {
    if (!matchedCategory) return;
    router.push({ pathname: '/new-request/create', params: { categoryId: matchedCategory.id } });
  };
  ...
  return (
    ...
    >
      ...
      <View style={styles.content}>
        {workers.length ? (
          workers.map((worker) => (
            <TouchableOpacity key={worker.id} style={styles.workerCard} onPress={() => router.push(`/provider/${worker.id}`)}>
              <Image source={worker.avatarUri} style={styles.workerAvatar} contentFit="cover" />
              <View style={styles.workerInfo}>
                <View style={styles.workerHeader}>
                  <Text style={theme.typography.h4}>{worker.name}</Text>
                  {worker.verified && <CheckCircle2 color={theme.colors.primary} size={16} style={{ marginLeft: 4 }} />}
                </View>
                <View style={styles.statsRow}>
                  <View style={styles.statItem}>
                    <Star color={theme.colors.warning} fill={theme.colors.warning} size={14} />
                    <Text style={[theme.typography.caption, { marginLeft: 3 }]}>{worker.rating.toFixed(1)} ({worker.reviewCount})</Text>
                  </View>
                  <View style={styles.statItem}>
                    <MapPin color={theme.colors.textSecondary} size={14} />
                    <Text style={[theme.typography.caption, { marginLeft: 3 }]}>{worker.distance}</Text>
                  </View>
                </View>
              </View>
              <View style={styles.priceContainer}>
                <Text style={[theme.typography.label, { color: theme.colors.primary }]}>{worker.price ?? 'Quote required'}</Text>
              </View>
            </TouchableOpacity>
          ))
        ) : (
          <View style={styles.emptyState}>
            ...
            <Text style={[theme.typography.h4, { color: theme.colors.textSecondary, marginTop: theme.spacing.md }]}>No workers online right now</Text>
            <Text style={[theme.typography.body2, { color: theme.colors.textTertiary, textAlign: 'center', marginTop: 4 }]}>Post a request and we will match an available {title} worker to you right away.</Text>
            <Button title="Book this service" fullWidth testID="category-book-cta" style={styles.bookButton} onPress={openRequestForCategory} />
          </View>
        )}
      </View>
    </Screen>
...
const styles = StyleSheet.create({
  ...
  bookButton: { marginTop: theme.spacing.lg, alignSelf: 'stretch' },
});
