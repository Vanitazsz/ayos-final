...
import type { useWorkerVerificationScreenController } from '../hooks/useWorkerVerificationScreenController';
      ...
      {steps.map((step, i) => (
        <View key={step.id} style={styles.stepRow}>
          ...
          <View
            style={[
              styles.stepBody,
              i < steps.length - 1 && styles.stepBodySpaced,
            ]}
          >
            ...
            <AppText
              variant="caption"
              color={
                step.status === 'pending' ? Colors.border : Colors.textSecondary
              }
            >
              {step.desc}
            </AppText>
...
function AlertCard({
  ...
  const border =
    type === 'warning' ? 'rgba(245,166,35,0.3)' : 'rgba(46,107,203,0.2)';
    ...
    <View
      style={[
        styles.card,
        {
          ...
          borderWidth: 1,
          borderColor: 'rgba(46,107,203,0.15)',
        },
      ]}
    >
      <View
...
function NextStepsCard() {
  const items = [
    ...
    {
      icon: <Briefcase size={16} color="#6B5BC5" />,
  ...
  return (
    <View style={styles.docSummary}>
      {counts.map((s) => (
        <View
          key={s.label}
          style={[styles.docSummaryCard, { backgroundColor: s.bg }]}
        >
          <AppText variant="h3" weight="bold" color={s.color}>
            {s.count}
          </AppText>
          <AppText variant="caption" weight="bold" color={s.color}>
            {s.label}
          </AppText>
        </View>
      ))}
...
function DocRow({ doc }: { doc: Document }) {
  const config = {
    verified: {
      ...
      bg: '#E7F8F5',
      icon: <CheckCircle size={12} color={Colors.verified} />,
    },
    uploaded: {
      label: 'In Review',
    ...
    rejected: {
      ...
      bg: '#FFF0F0',
      icon: <AlertCircle size={12} color={Colors.error} />,
    },
    missing: {
      label: 'Missing',
      textColor: Colors.textTertiary,
      bg: '#F0F4FA',
      icon: <Upload size={12} color={Colors.textTertiary} />,
    },
  }[doc.status];
    ...
    <View
      style={[
        ...
        (doc.status === 'rejected' || doc.status === 'missing') && {
          backgroundColor: doc.status === 'rejected' ? '#FFFAFA' : '#FAFAFA',
        },
      ]}
    >
      <View style={styles.docIcon}>
        {doc.status === 'missing' || doc.status === 'rejected' ? (
          <Upload
            size={16}
            color={
              doc.status === 'rejected' ? Colors.error : Colors.textTertiary
            }
          />
        ) : (
          <FileText
...
export function VerificationView({
  ...
  model: ReturnType<typeof useWorkerVerificationScreenController>;
}) {
  const {
    tab,
    setTab,
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.statusBanner}>
        ...
        <Pill
          label={`${steps.filter((step) => step.status === 'done').length} / ${steps.length}`}
          ...
          {(['status', 'documents', 'faq'] as const).map((t) => (
            ...
            >
              <AppText
                ...
                weight="bold"
                color={
                  tab === t ? Colors.textPrimary : 'rgba(255,255,255,0.45)'
                }
              >
                {t === 'status'
                  ? 'Status'
        ...
        {tab === 'status' && (
          <>
            ...
            <AlertCard
              type="warning"
              title={
                status === 'REJECTED' ? 'Action Required' : 'Application Status'
              }
              body={
          ...
          <>
            ...
            <Pressable
              ...
              onPress={() =>
                Alert.alert(
                  'Resubmit Documents',
                  'To resubmit documents, please contact support or re-register as a worker.',
                )
              }
            >
