...
select has_function('public','admin_decide_payout',array['uuid','text','text'],'payout decisions are protected');
...
select is((select count(*) from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relkind='r' and not c.relrowsecurity and c.relname <> 'wallet_topups'),0::bigint,'every public application table enables RLS');
