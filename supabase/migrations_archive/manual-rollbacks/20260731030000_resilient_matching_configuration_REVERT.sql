-- ROLLBACK SCRIPT: Revert 20260731030000_resilient_matching_configuration.sql
-- Run this script if you need to restore the exact previous database functions for worker matching.

BEGIN;

-- 1. Restore previous public.update_worker_presence function
...
BEGIN
  ...
  SELECT
    ...
    ROUND((extensions.st_y(p.location::extensions.geometry) + ((HASHTEXT(req.id::TEXT || wp.account_id::TEXT) % 17 - 8) * 0.00008))::NUMERIC, 6),
    ROUND((extensions.st_x(p.location::extensions.geometry) + ((HASHTEXT(wp.account_id::TEXT || req.id::TEXT) % 17 - 8) * 0.00008))::NUMERIC, 6),
