...
-- Let the customer rate the worker (1-5 stars + optional comment) from the
-- proof-of-work popup, alongside their proof photos.
--
-- The rating/comment reuse `bookings.worker_proof_rating` / `bookings.worker_proof_comment`
-- (the booking-level proof summary), so no new columns are needed. Previously the
-- customer branch of `attach_booking_proof` rejected rating/comment outright;
-- it now validates and persists them the same way the worker branch does.
...
begin
  ...
  if p_submitted_by in ('worker', 'customer')
