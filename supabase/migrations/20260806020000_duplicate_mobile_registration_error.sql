...
--
-- Adds a duplicate-mobile pre-check so that registering an account whose
-- mobile already exists on another account fails with a named
-- MOBILE_ALREADY_REGISTERED error (P0001) instead of surfacing the raw
-- unique-constraint violation (23505, accounts_mobile_key) from the
-- AFTER INSERT trigger, which aborts the surrounding transaction and
-- hides the actual reason from the client.
