...
export type MediaInput = { bucket: string; path: string; contentType: string; durationSeconds?: number };
...
async function loadMedia(admin: SupabaseClient, accountId: string, media: MediaInput[]) {
  if (media.length > 4) throw new HttpError(422,"invalid_media","At most three photos and one audio recording are allowed");
  ...
  if (images.length > 3 || audio.length > 1 || audio.some((item) => (item.durationSeconds ?? 61) > 60)) throw new HttpError(422,"invalid_media","Invalid photo or audio limits");
  const allowedBuckets = new Set(["service-request-media","request-media"]);
  return await Promise.all(media.map(async (item) => {
    if (!allowedBuckets.has(item.bucket) || item.path.split("/")[0] !== accountId) throw new HttpError(403,"invalid_media_owner","Media ownership validation failed");
    if (!/^(image\/(jpeg|png|webp)|audio\/(mpeg|mp4|m4a|wav|webm))$/.test(item.contentType)) throw new HttpError(422,"invalid_media_type","Unsupported media type");
    const { data, error } = await admin.storage.from(item.bucket).download(item.path); if (error || !data) throw new HttpError(422,"invalid_media","Media could not be read");
    const bytes = new Uint8Array(await data.arrayBuffer()); if (bytes.byteLength > 15*1024*1024) throw new HttpError(422,"media_too_large","Each media file must be 15 MB or smaller");
...
async function transcribeOpenAI(item: Awaited<ReturnType<typeof loadMedia>>[number]) {
  const key=Deno.env.get("OPENAI_API_KEY"); const model=Deno.env.get("OPENAI_TRANSCRIPTION_MODEL")??"gpt-4o-mini-transcribe-2025-12-15"; if(!key) throw Object.assign(new Error("OPENAI_NOT_CONFIGURED"),{retryable:true});
  const form=new FormData(); form.append("model",model); form.append("file",new Blob([item.bytes],{type:item.contentType}),`audio.${item.contentType.split("/")[1]}`);
  const response=await fetch("https://api.openai.com/v1/audio/transcriptions",{method:"POST",headers:{authorization:`Bearer ${key}`},body:form}); const body=await response.json().catch(()=>({}));
...
export async function runAnalysis(admin: SupabaseClient, accountId: string, description: string, mediaInput: MediaInput[]) {
  const [{data:categories,error:categoryError},{data:services,error:serviceError},media]=await Promise.all([
    admin.from("service_categories").select("id,name,minimum_price_minor,maximum_price_minor,is_safety_critical").eq("is_active",true),
    admin.from("services").select("id,category_id,name,minimum_price_minor,maximum_price_minor,is_safety_critical").eq("is_active",true),
...
export async function validateCatalogAndCosts(admin:SupabaseClient,result:AnalysisResult) {
  const [{data:categories},{data:services}]=await Promise.all([admin.from("service_categories").select("id,minimum_price_minor,maximum_price_minor,is_safety_critical").in("id",result.suggestedCategoryIds),admin.from("services").select("id,minimum_price_minor,maximum_price_minor,is_safety_critical").in("id",result.suggestedServiceIds)]);
  const categoryIds=new Set((categories??[]).map((row)=>row.id)); const serviceIds=new Set((services??[]).map((row)=>row.id)); result.suggestedCategoryIds=result.suggestedCategoryIds.filter((id)=>categoryIds.has(id)); result.suggestedServiceIds=result.suggestedServiceIds.filter((id)=>serviceIds.has(id));
  const bounds=[...(categories??[]),...(services??[])]; const minimum=Math.min(...bounds.map((row)=>row.minimum_price_minor).filter((v):v is number=>typeof v==="number")); const maximum=Math.max(...bounds.map((row)=>row.maximum_price_minor).filter((v):v is number=>typeof v==="number"));
  const costOutlier=Number.isFinite(minimum)&&Number.isFinite(maximum)&&(result.estimatedCostMinimumMinor<minimum||result.estimatedCostMaximumMinor>maximum); result.safetyCritical=result.safetyCritical||bounds.some((row)=>row.is_safety_critical);
