...
async function pdf(rows: Record<string, unknown>[], title: string) {
  const document = await PDFDocument.create();
  const font = await document.embedFont(StandardFonts.Helvetica);
  let page = document.addPage([842, 595]);
  let y = 565;
  page.drawText(title, { x: 30, y, size: 16, font });
  y -= 24;
  for (const row of rows.slice(0, 1000)) {
    ...
    page.drawText(line, { x: 30, y, size: 7, font });
    y -= 10;
  }
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: isAdmin } = await client.rpc('is_admin', { require_aal2: true });
    if (!isAdmin)
    ...
    const { data: exportRow, error: createError } = await admin
      ...
      .insert({
        ...
        parameters: { format, filters: body.filters ?? {} },
        status: 'PROCESSING',
    ...
    try {
      ...
      if (format === 'CSV') {
        ...
        bytes = await pdf(rows, `A-yos ${reportType} Report`);
        contentType = 'application/pdf';
      }
      const path = `${user.id}/${exportRow.id}.${extension}`;
      ...
      const { data: complete, error: completeError } = await admin
        .from('report_exports')
        .update({ storage_path: path, status: 'COMPLETED', completed_at: new Date().toISOString() })
        .eq('id', exportRow.id)
        .select()
        .single();
      if (completeError) throw completeError;
      return success(complete, 'Report generated', 201);
    } catch (error) {
      await admin
        .from('report_exports')
        .update({
          status: 'FAILED',
          failure_reason: error instanceof Error ? error.message : 'Report failed',
