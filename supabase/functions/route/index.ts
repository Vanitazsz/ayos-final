  ...
  try {
    ...
    if (start.length !== 2 || end.length !== 2)
      throw new HttpError(
        422,
        'invalid_coordinates',
        'start and end must be [longitude, latitude]',
      );
    assertPhilippines(start[1], start[0]);
    assertPhilippines(end[1], end[0]);
    const bookingId = typeof body.bookingId === 'string' ? body.bookingId : null;
    if (bookingId) {
      const { data: allowed } = await client.rpc('is_booking_party', { target_booking: bookingId });
      const { data: isAdmin } = await client.rpc('is_admin', { require_aal2: false });
      if (!allowed && !isAdmin)
        throw new HttpError(403, 'forbidden', 'Booking route access denied');
    }
    ...
    if (bookingId && !result.cached)
      await admin.from('route_snapshots').insert({
        booking_id: bookingId,
        requested_by: user.id,
        route_geojson: result.value.geojson,
        distance_meters: result.value.distanceMeters,
        duration_seconds: result.value.durationSeconds,
        worker_location: `SRID=4326;POINT(${start[0]} ${start[1]})`,
        destination: `SRID=4326;POINT(${end[0]} ${end[1]})`,
      });
    return success({
      ...result.value,
