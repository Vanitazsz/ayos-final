...
async function main() {
  ...
  for (const [code, permissions] of Object.entries(roleDefinitions)) {
    const role = await prisma.role.upsert({
      ...
      create: {
        code,
        name: code.replaceAll('_', ' ').replace(/\b\w/g, (value) => value.toUpperCase()),
      },
    });
  ...
  const serviceSeeds = [
    ['plumbing', 'Pipe Repair', 4500, 90],
    ['plumbing', 'Drain Cleaning', 5000, 90],
    ['electrical', 'Electrical Inspection', 5500, 120],
    ['hvac', 'AC Repair', 6500, 120],
    ['cleaning', 'Deep Cleaning', 3500, 180],
    ['carpentry', 'Furniture Assembly', 4000, 120],
    ['painting', 'Interior Painting', 5000, 240],
    ['repair', 'General Repair', 4500, 120],
  ] as const;
  ...
  const admin = await upsertUser(
    process.env.SEED_ADMIN_EMAIL ?? 'admin@a-yos.local',
    process.env.SEED_ADMIN_PASSWORD ?? 'ChangeMe-Admin-123!',
    'Super',
    'Admin',
    ['super_admin'],
  );
  await upsertUser(
    process.env.SEED_CUSTOMER_EMAIL ?? 'customer@a-yos.local',
    process.env.SEED_CUSTOMER_PASSWORD ?? 'ChangeMe-Customer-123!',
    'Juan',
    'Dela Cruz',
    ['customer'],
  );
  const workerUser = await upsertUser(
    process.env.SEED_WORKER_EMAIL ?? 'worker@a-yos.local',
    process.env.SEED_WORKER_PASSWORD ?? 'ChangeMe-Worker-123!',
    'Carlos',
    'Mendez',
    ['customer', 'worker'],
  );
  ...
  const plumbingSkills = await prisma.skill.findMany({
    where: { industryId: plumbing.id },
    take: 4,
  });
