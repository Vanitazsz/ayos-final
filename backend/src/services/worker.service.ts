...
function providerView(worker: any, distanceKm?: number) {
  const name = [
    worker.user.profile?.firstName,
    worker.user.profile?.middleName,
    worker.user.profile?.lastName,
  ]
    .filter(Boolean)
    .join(' ');
  ...
  return {
    ...
    avatarUri:
      worker.user.profile?.avatar?.visibility === 'PUBLIC'
        ? `/api/v1/uploads/${worker.user.profile.avatar.id}`
        : null,
    ...
    portfolioImages: worker.portfolio
      .filter((entry: any) => entry.upload.visibility === 'PUBLIC')
      .map((entry: any) => `/api/v1/uploads/${entry.upload.id}`),
  };
}
...
export class WorkerService {
  async providers(query: ProviderQuery) {
    ...
    let views = workers.map((worker) => {
      const area = worker.serviceAreas.find(
        (item) => item.latitude !== null && item.longitude !== null,
      );
      const distance =
        query.lat !== undefined && query.lng !== undefined && area
          ? haversine(query.lat, query.lng, Number(area.latitude), Number(area.longitude))
          : undefined;
      return providerView(worker, distance);
    });
    if (query.radiusKm !== undefined)
      views = views.filter(
        (view) => view.distanceKm !== null && view.distanceKm <= query.radiusKm!,
      );
    views.sort((a, b) =>
      query.sort === 'distance'
        ? (a.distanceKm ?? Infinity) - (b.distanceKm ?? Infinity)
        : query.sort === 'price_asc'
          ? (a.hourlyRate ?? Infinity) - (b.hourlyRate ?? Infinity)
  ...
  async provider(id: string) {
    ...
    const reviews = await prisma.review.findMany({
      where: { subjectId: worker.userId, status: 'PUBLISHED', deletedAt: null },
  ...
  async availability(id: string, from: Date, to: Date) {
    const worker = await prisma.workerProfile.findFirst({
      where: { OR: isUuid(id) ? [{ id }, { userId: id }] : [], verificationStatus: 'APPROVED' },
    });
    if (!worker) throw new NotFoundError('Provider');
    ...
    return {
      from,
      to,
      unavailable: bookings.map((booking) => booking.scheduledAt),
      slotMinutes: 60,
    };
  }
  ...
  async update(userId: string, data: Prisma.WorkerProfileUpdateInput) {
    if (typeof data.contactPhone === 'string')
      data.contactPhone = normalizePhone(data.contactPhone);
  ...
  async replaceSkills(userId: string, skillValues: string[]) {
    ...
    const skills = await prisma.skill.findMany({
      where: { OR: [{ id: { in: skillValues.filter(isUuid) } }, { code: { in: skillValues } }] },
    });
    ...
    await prisma.$transaction(async (transaction) => {
      ...
      await transaction.workerSkill.createMany({
        data: skills.map((skill) => ({ workerId: worker.id, skillId: skill.id })),
      });
    });
  ...
  async replaceAreas(
    userId: string,
    areas: Array<{ label: string; latitude?: number; longitude?: number; radiusKm?: number }>,
  ) {
    const worker = await prisma.workerProfile.findUnique({ where: { userId } });
    if (!worker) throw new NotFoundError('Worker profile');
    await prisma.$transaction(async (transaction) => {
      ...
      if (areas.length)
        await transaction.workerServiceArea.createMany({
          data: areas.map((area) => ({ workerId: worker.id, ...area })),
        });
    });
  ...
  async submit(userId: string) {
    ...
    if (!worker) throw new NotFoundError('Worker profile');
    if (!['DRAFT', 'NEEDS_DOCUMENTS', 'REJECTED'].includes(worker.verificationStatus))
      throw new ConflictError(
        'Worker application is already under review or approved',
        'invalid_worker_state',
      );
    const documentTypes = new Set(
      worker.documents
        .filter((document) => document.status !== 'REJECTED')
        .map((document) => `${document.type}:${document.side ?? ''}`),
    );
    if (!documentTypes.has('government_id:front') || !documentTypes.has('government_id:back'))
      throw new ConflictError(
        'Front and back government ID documents are required',
        'documents_required',
      );
    if (!worker.skills.length)
      throw new ConflictError('At least one skill is required', 'skills_required');
    return prisma.workerProfile.update({
      where: { id: worker.id },
      data: { verificationStatus: 'PENDING', submittedAt: new Date(), rejectionReason: null },
    });
  }
  ...
  async verification(userId: string) {
    const worker = await prisma.workerProfile.findUnique({
      ...
      include: {
        documents: {
          include: { upload: { select: { id: true, originalName: true, createdAt: true } } },
          orderBy: { createdAt: 'desc' },
        },
      },
    });
    ...
    return {
      ...
      activationFeePaidAt: worker.activationFeePaidAt,
  ...
  async dashboard(userId: string) {
    ...
    const [active, pending, completed, upcoming] = await Promise.all([
      prisma.booking.count({
        where: { workerId: userId, status: { in: ['EN_ROUTE', 'ARRIVED', 'IN_PROGRESS'] } },
      }),
      ...
      prisma.booking.findMany({
        where: { workerId: userId, status: { notIn: ['COMPLETED', 'CANCELLED', 'REFUNDED'] } },
    ...
    return {
      stats: {
        ...
        pending,
        completed,
        earnings: worker.wallet?.balance ?? 0,
        currency: worker.wallet?.currency ?? 'PHP',
      },
      performance: { completionRate: null, onTimeArrival: null, repeatClients: null },
      activeBookings: upcoming,
    };
  }

  async addDocument(userId: string, input: { uploadId: string; type: string; side?: string }) {
    const worker = await prisma.workerProfile.findUnique({ where: { userId } });
    if (!worker) throw new NotFoundError('Worker profile');
    ...
    if (!upload) throw new NotFoundError('Worker document upload');
    return prisma.$transaction(async (tx) => {
      const document = await tx.verificationDocument.create({
        data: { workerId: worker.id, uploadId: input.uploadId, type: input.type, side: input.side },
      });
      await tx.upload.update({ where: { id: input.uploadId }, data: { attachedAt: new Date() } });
      return document;
    });
  }
  async wallet(userId: string) {
    ...
    const wallet =
      worker.wallet ??
      (await prisma.wallet.create({ data: { workerId: worker.id, currency: worker.currency } }));
    ...
    return {
      ...wallet,
      totals: Object.fromEntries(totals.map((x) => [x.direction, x._sum.amount ?? 0])),
    };
  }
  async walletTransactions(userId: string, page: number, limit: number) {
    const worker = await prisma.workerProfile.findUnique({
      where: { userId },
      include: { wallet: true },
    });
    if (!worker) throw new NotFoundError('Worker profile');
    if (!worker.wallet) return { items: [], meta: paginationMeta(page, limit, 0) };
    const where = { walletId: worker.wallet.id };
  ...
  async payoutMethods(userId: string) {
    ...
    return rows.map(({ providerToken: _, ...row }) => row);
  }
  async createPayoutMethod(userId: string, input: any) {
    const worker = await prisma.workerProfile.findUnique({ where: { userId } });
    if (!worker) throw new NotFoundError('Worker profile');
    const { setDefault, ...data } = input;
    return prisma.$transaction(async (tx) => {
      if (setDefault)
        await tx.payoutMethod.updateMany({
          where: { workerId: worker.id, isDefault: true },
          data: { isDefault: false },
        });
      const row = await tx.payoutMethod.create({
        data: { workerId: worker.id, ...data, isDefault: setDefault },
      });
      const { providerToken: _, ...safe } = row;
      return safe;
    });
  }
  async payouts(userId: string, page: number, limit: number) {
    const worker = await prisma.workerProfile.findUnique({ where: { userId } });
    if (!worker) throw new NotFoundError('Worker profile');
    ...
    const [items, total] = await prisma.$transaction([
      prisma.payout.findMany({
        ...
        include: {
          payoutMethod: {
            select: { id: true, type: true, provider: true, label: true, accountMask: true },
          },
        },
        orderBy: { createdAt: 'desc' },
      }),
  ...
  async createPayout(userId: string, input: any) {
    const worker = await prisma.workerProfile.findUnique({
      where: { userId },
      include: { wallet: true },
    });
    if (!worker?.wallet) throw new ConflictError('Wallet is not available', 'wallet_unavailable');
    if (worker.wallet.currency !== input.currency)
      throw new ConflictError('Payout currency does not match wallet', 'currency_mismatch');
    const method = await prisma.payoutMethod.findFirst({
      where: { id: input.payoutMethodId, workerId: worker.id, deletedAt: null },
    });
    ...
    return prisma.$transaction(async (tx) => {
      const changed = await tx.wallet.updateMany({
        where: { id: worker.wallet!.id, balance: { gte: input.amount } },
        data: { balance: { decrement: input.amount }, version: { increment: 1 } },
      });
      if (!changed.count)
        throw new ConflictError('Insufficient wallet balance', 'insufficient_balance');
