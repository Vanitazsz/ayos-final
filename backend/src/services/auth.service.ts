...
function parseBirthday(value: string): Date {
  if (/^\d{2}\/\d{2}\/\d{4}$/.test(value)) {
    ...
    if (
      ...
      throw new ConflictError('Birthday is invalid', 'invalid_birthday');
    return date;
  }
...
export class AuthService {
  ...
  async register(input: RegisterInput, client: ClientInfo) {
    ...
    if ((await authRepository.findByEmail(email)) || (await authRepository.findByPhone(phone)))
      throw new ConflictError('Email or mobile number is already registered', 'account_exists');
    ...
    const user = await prisma.$transaction(async (transaction) => {
      ...
      await transaction.otpChallenge.create({
        data: {
          ...
          expiresAt: expiresInSeconds(env.OTP_TTL_SECONDS),
        },
      });
    ...
    return {
      ...
      ...(env.NODE_ENV !== 'production'
        ? { developmentOtp: user.otp, developmentEmailVerificationToken: user.verificationToken }
        : {}),
    };
  }
  ...
  async registerWorker(input: WorkerRegistrationInput, client: ClientInfo) {
    ...
    const industry =
      ...
      (await prisma.industry.create({
        data: { code: industryCode, name: input.industry, isActive: false },
      }));
    ...
    for (const skillValue of input.skills) {
      ...
      skill ??= await prisma.skill.create({
        data: { industryId: industry.id, code, name: skillValue, isActive: false },
      });
      skillRows.push(skill);
    }
    const gender = input.gender
      ? ({ male: Gender.MALE, female: Gender.FEMALE, other: Gender.OTHER } as const)[input.gender]
      : undefined;
    ...
    return {
      ...
      ...(await this.issueTokens(authUser, client)),
      ...(env.NODE_ENV !== 'production'
        ? { developmentOtp: userId.otp, developmentEmailVerificationToken: verificationToken }
        : {}),
    };
  }
  ...
  async login(input: LoginInput, client: ClientInfo) {
    ...
    const user = login.includes('@')
      ? await authRepository.findByEmail(normalizeEmail(login))
      : await authRepository.findByPhone(normalizePhone(login));
    if (
      !user ||
      user.deletedAt ||
  ...
  async refresh(refreshToken: string, client: ClientInfo) {
    ...
    await prisma.$transaction(async (transaction) => {
      ...
      const updated = await transaction.session.updateMany({
        ...
        data: { revokedAt: new Date(), replacedBySessionId: newSessionId, lastUsedAt: new Date() },
      });
      if (updated.count !== 1)
        throw new AuthenticationError('Refresh token was already used', 'refresh_token_reuse');
    });
  ...
  async forgotPassword(emailInput: string) {
    ...
    await prisma.verificationToken.create({
      data: {
        ...
        expiresAt: expiresInSeconds(env.RESET_TOKEN_TTL_SECONDS),
      },
    });
  ...
  async resendEmailVerification(userId: string) {
    ...
    if (user.emailVerifiedAt)
      throw new ConflictError('Email is already verified', 'already_verified');
    const token = createOpaqueToken();
    await prisma.$transaction([
  ...
  async verifyPhone(userId: string, code: string) {
    const challenge = await prisma.otpChallenge.findFirst({
      where: { userId, purpose: OtpPurpose.PHONE_VERIFICATION, consumedAt: null },
  ...
  async resendOtp(userId: string) {
    const latest = await prisma.otpChallenge.findFirst({
      where: { userId, purpose: OtpPurpose.PHONE_VERIFICATION },
      orderBy: { createdAt: 'desc' },
    });
    if (latest && Date.now() - latest.createdAt.getTime() < 30_000)
      throw new ConflictError('Please wait before requesting another code', 'otp_cooldown');
    const otp = createOtp();
