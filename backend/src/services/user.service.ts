...
export class UserService {
  async me(userId: string) {
    const user = await prisma.user.findFirst({
      ...
      select: {
        ...
        createdAt: true,
        profile: true,
        workerProfile: { select: { id: true, verificationStatus: true, availabilityStatus: true } },
  ...
  async deleteAccount(userId: string, password: string) {
    ...
    await prisma.$transaction(async (transaction) => {
      await transaction.user.update({
        where: { id: userId },
        data: { deletedAt: now, status: 'DISABLED', tokenVersion: { increment: 1 } },
      });
      await transaction.session.updateMany({
        where: { userId, revokedAt: null },
        data: { revokedAt: now },
      });
      await transaction.deletionRecord.create({
        data: {
          entityType: 'User',
          entityId: userId,
          displayName: user.email,
          deletedById: userId,
          purgeAfter: new Date(now.getTime() + 30 * 86400000),
        },
      });
    });
  }
  ...
  async deleteAddress(userId: string, id: string) {
    ...
    if (address.isDefault)
      throw new ConflictError(
        'Set another default address before deleting this one',
        'default_address',
      );
  ...
  async addIdentityDocument(
    userId: string,
    input: { uploadId: string; type: string; side?: string },
  ) {
    const upload = await prisma.upload.findFirst({
      where: {
        id: input.uploadId,
    ...
    if (!upload) throw new NotFoundError('Identity upload');
    return prisma.$transaction(async (tx) => {
      const document = await tx.verificationDocument.create({
        data: { userId, uploadId: input.uploadId, type: input.type, side: input.side },
      });
  ...
  async listFavorites(userId: string) {
    return prisma.favorite.findMany({
      where: { userId },
      include: {
        provider: { include: { profile: true, workerProfile: { include: { industry: true } } } },
      },
  ...
  async addFavorite(userId: string, providerId: string) {
    const provider = await prisma.user.findFirst({
      where: { id: providerId, workerProfile: { verificationStatus: 'APPROVED' }, deletedAt: null },
    });
