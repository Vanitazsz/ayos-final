...
const phPhone = z
  .string()
  .trim()
  .regex(/^(09|\+639)\d{9}$/, 'Use a valid Philippine mobile number');
...
export const loginSchema = z
  ...
  .strict()
  .refine((value) => Boolean(value.emailOrPhone || value.email), {
    message: 'Email or phone is required',
    path: ['emailOrPhone'],
  });

export const refreshSchema = z
  .object({ refreshToken: z.string().min(32).max(500).optional() })
  .strict();
