...
const requestFieldsSchema = z.object({
  ...
  hasParts: z.boolean().optional(),
  partsDescription: z.string().trim().max(1000).optional(),
  attachmentIds: z.array(uuid).max(10).default([]),
});
export const requestCreateSchema = requestFieldsSchema.refine(
  (v) => v.budgetMin === undefined || v.budgetMax === undefined || v.budgetMin <= v.budgetMax,
  { path: ['budgetMax'], message: 'budgetMax must be at least budgetMin' },
);
export const requestUpdateSchema = requestFieldsSchema
  .partial()
...
export const publishSchema = z.object({
  urgency: z.enum(['ASAP', 'THIS_WEEK', 'OPEN_BIDDING']),
...
export const bidSchema = z
  ...
  .refine((v) => v.minAmount <= v.maxAmount, {
    path: ['maxAmount'],
    message: 'maxAmount must be at least minAmount',
  });
...
export const cancelSchema = z.object({
  stage: z.enum(['BEFORE_TRAVELING', 'AFTER_ARRIVING', 'AFTER_INSPECTING']),
  reasonId: z.string().trim().min(1).max(20),
  customReason: z.string().trim().max(1000).optional(),
  notes: z.string().trim().max(2000).optional(),
});
export const bookingUpdateSchema = bookingCreateSchema
  .omit({ providerId: true, serviceId: true })
  .partial()
  .extend({ version: z.number().int().positive() });
