...
export const conversationCreate = z.object({
  participantIds: z.array(uuid).min(1).max(10),
...
export const messageCreate = z
  ...
  .refine(
    (v) => Boolean(v.text) || v.attachmentIds.length > 0,
    'A message or attachment is required',
  );
export const conversationList = paginationSchema.extend({
  search: z.string().trim().max(100).optional(),
});
export const readMessage = z.object({ messageId: uuid.optional() });
export const reviewList = paginationSchema.extend({
...
export const reviewCreate = z.object({
  ...
  photoUploadIds: z.array(uuid).max(6).default([]),
});
export const reviewUpdate = reviewCreate.omit({ bookingId: true }).partial();
export const helpful = z.object({ helpful: z.boolean() });
export const reviewReport = z.object({
  reason: z.string().trim().min(1).max(200),
  details: z.string().trim().max(2000).optional(),
});
export const notificationList = paginationSchema.extend({
  unread: z.coerce.boolean().optional(),
  type: z.string().max(64).optional(),
});
export const ticketList = paginationSchema.extend({
...
export const reviewReply = z.object({ message: z.string().trim().min(1).max(2000) });
...
export const resolution = z.object({ resolution: z.string().trim().max(2000).optional() });
