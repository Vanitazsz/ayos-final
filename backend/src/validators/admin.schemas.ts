...
export const adminList = paginationSchema.extend({
...
export const workerDecision = z.object({ remarks: z.string().trim().max(2000).optional() });
...
export const settingsBody = z.record(z.string().min(1).max(120), z.unknown());
export const campaignBody = z.object({
  title: z.string().trim().min(1).max(200),
  audience: z.enum(['ALL_USERS', 'WORKERS_ONLY', 'CUSTOMERS_ONLY', 'INACTIVE_USERS']),
