...
export const workerSkillsSchema = z
  .object({ skills: z.array(z.string().trim().min(1).max(120)).min(1).max(30) })
  .strict();
export const workerAreasSchema = z
  .object({
    areas: z
      .array(
        z
...
export const payoutMethodSchema = z.object({
  ...
  providerToken: z.string().trim().min(1).max(255),
  label: z.string().trim().min(1).max(100),
  accountMask: z.string().trim().min(1).max(100),
