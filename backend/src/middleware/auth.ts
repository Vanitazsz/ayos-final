...
export const authenticate: RequestHandler = async (request, _response, next) => {
  try {
    ...
    const session = await prisma.session.findUnique({
      ...
      include: {
        user: { select: { id: true, status: true, tokenVersion: true, deletedAt: true } },
      },
    });
    if (
      !session ||
    ...
    if (
      ...
      session.user.tokenVersion !== context.tokenVersion
    ) {
...
export function requirePermission(...permissions: string[]): RequestHandler {
  return (request, _response, next) => {
    ...
    if (
      ...
      permissions.every((permission) => request.auth!.permissions.includes(permission))
    ) {
      return next();
    }
...
export function requireRole(...roles: string[]): RequestHandler {
  return (request, _response, next) => {
    ...
    if (
      request.auth.roles.includes('super_admin') ||
      roles.some((role) => request.auth!.roles.includes(role))
    )
      return next();
    return next(new AuthorizationError());
  };
