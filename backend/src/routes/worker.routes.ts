...
import {
  ...
  workerSkillsSchema,
} from '../validators/worker.schemas.js';
...
providerRouter.get(
  '/:id/availability',
  validate({ params: uuidParamsSchema, query: availabilityQuerySchema }),
...
workerRouter.put(
  '/me/service-areas',
  validate({ body: workerAreasSchema }),
  workerController.areas,
);
...
workerRouter.post(
  '/me/documents',
  validate({ body: workerDocumentSchema }),
  workerController.document,
);
workerRouter.get('/me/wallet', workerController.wallet);
workerRouter.get(
  '/me/wallet/transactions',
  validate({ query: paginationSchema }),
  workerController.walletTransactions,
);
workerRouter.get('/me/payout-methods', workerController.payoutMethods);
workerRouter.post(
  '/me/payout-methods',
  validate({ body: payoutMethodSchema }),
  workerController.createPayoutMethod,
);
