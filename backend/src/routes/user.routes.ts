...
router.get(
  ...
  validate({ query: addressListSchema }),
...
userCompatibilityRouter.put(
  '/location',
  validate({ body: addressSchema.omit({ label: true, isDefault: true, countryCode: true }) }),
  userController.setLocation,
);
userCompatibilityRouter.post('/consent', validate({ body: consentSchema }), userController.consent);
userCompatibilityRouter.post(
  '/verify-identity',
