...
try {
  const existingTerms = await admin
    .from('content_pages')
    .select('id')
    .eq('key', 'TERMS')
    .maybeSingle();
  ...
  assert(
    ...
    `Current profile RPC returned no role profile: ${JSON.stringify(initialProfile.data)}`,
  );
  assert(
    initialProfile.data.profile.display_name === 'Smoke Test',
    'Profile provisioning did not preserve the submitted name',
  );
  ...
  assert(
    updatedProfile.data.profile.display_name === 'Smoke Test Updated',
    'Profile update RPC did not persist the name',
  );

  const avatarPath = `${userId}/smoke-avatar.png`;
  const avatarUpload = await client.storage
    .from('profile-avatars')
    .upload(avatarPath, new Blob(['avatar'], { type: 'image/png' }), { contentType: 'image/png' });
  ...
  const createdWorker = await client
  ...
  assert(
    createdWorker.data.display_name === 'Smoke Test Updated',
    'Role switching generated a placeholder profile name',
  );
  ...
  const uploaded = await client.storage
    ...
    .upload(`${userId}/smoke.png`, new Blob(['smoke'], { type: 'image/png' }), {
