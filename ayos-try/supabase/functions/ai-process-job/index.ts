...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: job, error } = await admin
      ...
      .select('*,ai_processing_consents(*)')
      .eq('id', jobId)
      .eq('account_id', user.id)
      .single();
    if (error || !job) throw new HttpError(404, 'job_not_found', 'AI job not found');
    if (job.status === 'SUCCEEDED') return success(job, 'AI analysis already completed');
    if (!['QUEUED', 'FAILED'].includes(job.status))
      throw new HttpError(409, 'job_not_processable', 'AI job is already processing');
    if (job.ai_processing_consents?.revoked_at)
      throw new HttpError(422, 'consent_revoked', 'AI consent was revoked');
    await admin
      .from('ai_analysis_jobs')
      .update({
        status: 'PROCESSING',
    ...
    try {
      const output = await runAnalysis(
        ...
        user.id,
        job.description,
        job.media_paths as MediaInput[],
      );
      const checked = await validateCatalogAndCosts(admin, output.result);
      const firstMedia = output.media[0]?.path ?? null;
      const inputType = output.media.some((m) => m.contentType.startsWith('audio/'))
        ? 'VOICE'
        : output.media.length
          ? 'IMAGE'
          : 'TEXT';
      ...
      const { data: analysis, error: persistError } = await admin.rpc('persist_ai_analysis', {
        p_account_id: user.id,
        p_input_type: inputType,
        p_input_storage_path: firstMedia,
      ...
      const result = {
        ...checked.result,
        costOutlier: checked.costOutlier,
      ...
      throw new HttpError(
        ...
        value.retryable
          ? 'AI processing failed; retry or continue manually'
          : 'AI processing rejected this input; continue manually',
      );
    }
  } catch (error) {
