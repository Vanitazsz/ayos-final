...
begin
  select * into v_request from service_requests where id=p_service_request_id and user_account_id=auth.uid();
...
begin
  ...
  select jsonb_build_object('serviceRequestId',req.id,'startedAt',session_row.started_at,'expiresAt',session_row.expires_at,
    ...
    'diagnostics',private.live_dispatch_diagnostics(req.id,1::smallint),
