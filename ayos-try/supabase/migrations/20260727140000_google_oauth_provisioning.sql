...
-- Google OAuth does not carry the custom role metadata supplied by the
-- password registration forms. First-time Google identities are customer
-- accounts; existing worker identities retain the role already provisioned
-- for their auth user.
...
begin
  ...
  if bootstrap_request.email is not null then
    ...
    requested_role_text := upper(
      btrim(coalesce(new.raw_user_meta_data->>'role', ''))
    );
    ...
    then
      ...
      begin
        requested_role := requested_role_text::public.account_role;
      exception
